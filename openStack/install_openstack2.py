...
def install_database():
    ...
    execute("/etc/init.d/mysql restart", True)
    time.sleep(2)
    try:
...
def install_and_configure_keystone():
    ...
    execute("sudo service keystone restart", True)
...
def install_and_configure_glance():
    ...
    execute("sudo service glance-api restart", True)
    execute("sudo service glance-registry restart", True)
...
def install_and_configure_nova():
    ...
    add_to_conf(nova_conf, "DEFAULT", "glance_api_servers", "%s:9292" % ip_address_mgmt)
    ...
    add_to_conf(nova_conf, "DEFAULT", "neutron_admin_auth_url", "http://%s:5000/v2.0/" % ip_address_mgmt)
    add_to_conf(nova_conf, "DEFAULT", "neutron_auth_strategy", "keystone")
    add_to_conf(nova_conf, "DEFAULT", "neutron_url", "http://%s:9696/" % ip_address_mgmt)
    ...
    execute("sudo service nova-api restart", True)
    execute("sudo service nova-cert restart", True)
    execute("sudo service nova-scheduler restart", True)
    execute("sudo service nova-conductor restart", True)
    execute("sudo service nova-consoleauth restart", True)
    execute("sudo service nova-novncproxy restart", True)
    execute("sudo service libvirt-bin restart", True)
    execute("sudo service nova-compute restart", True)
...
def install_and_configure_neutron():
    ...
    add_to_conf(neutron_plugin_conf, "ml2_odl", "url", "http://%(odl_ip)s:%(odl_port)s/controller/nb/v2/neutron" % {'odl_ip': odl_ip, 'odl_port': odl_port})
    add_to_conf(neutron_plugin_conf, "ml2", "mechanism_drivers", "opendaylight, logger")
    ...
    execute("sudo service neutron-server restart", True)
    execute("sudo service neutron-dhcp-agent restart", True)
    execute("sudo service neutron-l3-agent restart", True)
